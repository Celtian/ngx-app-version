import {
  ...
  NgxAppVersionBreakpointsOrPredefinedBreakpoints,
  NgxAppVersionOptionsResponsiveSizes,
  NgxAppVersionPredefinedBreakpoints,
  NgxAppVersionResponsiveSize,
  NgxAppVersionResponsiveSizeOrNumber,
  NgxAppVersionResponsiveSizes,
  NgxAppVersionSizes,
  NgxAppVersionSizesOnlyResponsive,
  NgxAppVersionStyles,
  NgxAppVersionStylesBreakpointData
 ...
 */
export const coerceNgxAppVersionSizes = (value: any): NgxAppVersionSizes => {
 ...
 * @returns object
 */
export const normalizeResponsiveSize = (value: NgxAppVersionResponsiveSizeOrNumber): NgxAppVersionResponsiveSize => {
  if (!value) {
    ...
    const v = value as NgxAppVersionResponsiveSize;
    v.xs = v.xs < 1 ? 1 : v.xs;
    v.sm = v.sm < 1 ? 1 : v.sm;
    v.md = v.md < 1 ? 1 : v.md;
    v.lg = v.lg < 1 ? 1 : v.lg;
 ...
 */
export const normalizeAllResponsiveSizes = (value: NgxAppVersionOptionsResponsiveSizes): NgxAppVersionResponsiveSizes => {
  if (!value) {
    return undefined;
  }
  return {
 ...
 */
export const selectBreakpoints = (breakpoints: NgxAppVersionBreakpointsOrPredefinedBreakpoints): NgxAppVersionBreakpoints => {
  if (breakpoints && typeof breakpoints === 'string') {
    const bp = String(breakpoints) as NgxAppVersionPredefinedBreakpoints;
    switch (bp) {
      case 'BOOTSTRAP':
...
export const extractStyleSheetData = (
  breakpoints: NgxAppVersionBreakpoints,
  responsiveSizes: NgxAppVersionResponsiveSizes
): NgxAppVersionStyles => {
 ...
 */
export const createCss = (key: NgxAppVersionSizesOnlyResponsive, data: NgxAppVersionStylesBreakpointData): string => {
  const sheet: string[] = [];
