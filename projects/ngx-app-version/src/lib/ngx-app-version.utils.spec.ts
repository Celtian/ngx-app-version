...
import {
  NgxAppVersionBreakpoints,
  NgxAppVersionResponsiveSizes,
  NgxAppVersionStyles
} from './ngx-app-version-options.interface';
import {
...
} from './ngx-app-version.constants';
import {
  coerceIntProperty,
  coerceNgxAppVersionSizes,
  createCss,
  ...
  selectBreakpoints
} from './ngx-app-version.utils';

describe('NgxAppVersionUtils', () => {
  ...
  describe('coerceNgxAppVersionSizes', () => {

    it('should coerce "xs" to "xs"', () => {
      expect(coerceNgxAppVersionSizes('xs')).toEqual('xs');
    });

    it('should coerce "sm" to "sm"', () => {
      expect(coerceNgxAppVersionSizes('sm')).toEqual('sm');
    });

    it('should coerce "md" to "md"', () => {
      expect(coerceNgxAppVersionSizes('md')).toEqual('md');
    });

    it('should coerce "lg" to "lg"', () => {
      expect(coerceNgxAppVersionSizes('lg')).toEqual('lg');
    });

    it('should coerce "xl" to "xl"', () => {
      expect(coerceNgxAppVersionSizes('xl')).toEqual('xl');
    });

    it('should coerce 1 to 1', () => {
      expect(coerceNgxAppVersionSizes(1)).toEqual(1);
    });

    it('should coerce 2 to 2', () => {
      expect(coerceNgxAppVersionSizes(2)).toEqual(2);
    });

    it('should coerce -1 or 0 to 1', () => {
      expect(coerceNgxAppVersionSizes(-1)).toEqual(1);
      expect(coerceNgxAppVersionSizes(0)).toEqual(1);
    });

    it('should coerce null or undefined into 1', () => {
      expect(coerceNgxAppVersionSizes(null)).toEqual(1);
      expect(coerceNgxAppVersionSizes(undefined)).toEqual(1);
    });
  });
  ...
  describe('selectBreakpoints', () => {
    ...
    it('should select custom breakpoints', () => {
      const customBreakpoints: NgxAppVersionBreakpoints = {
        sm: 200,
        md: 300,
        lg: 400,
        xl: 500
  ...
  describe('extractStyleSheetData', () => {
    ...
    it('should convert responsive sizes & breakpoints into single object', () => {
      const responsiveSizes: NgxAppVersionResponsiveSizes = DEFAULT_RESPONSIVE_SIZES;
      const breakpoints: NgxAppVersionBreakpoints = DEFAULT_BREAKPOINTS;
  ...
  describe('createCss', () => {
    ...
    it('should return xs stylesheet', () => {
      expect(createCss('xs', data.xs)).toContain('.ngx-app-version-xs');
    });

    it('should return sm stylesheet', () => {
      expect(createCss('sm', data.sm)).toContain('.ngx-app-version-sm');
    });

    it('should return md stylesheet', () => {
      expect(createCss('md', data.md)).toContain('.ngx-app-version-md');
    });

    it('should return lg stylesheet', () => {
      expect(createCss('lg', data.lg)).toContain('.ngx-app-version-lg');
    });

    it('should return xl stylesheet', () => {
      expect(createCss('xl', data.xl)).toContain('.ngx-app-version-xl');
    });
  });
});
